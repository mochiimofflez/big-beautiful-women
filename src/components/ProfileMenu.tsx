    ...
    <div className='relative' ref={menuRef}>
      ...
      {isOpen && (
        <div className='absolute right-0 top-16 w-48 bg-[#1c1a1a] border border-brass/10 rounded-2xl p-2 z-[9999]' style={{ display: 'block' }}> 
          ...
          <button type='button' onClick={() => { auth.logout(); setIsOpen(false); navigate('/Login'); }} className='block w-full text-left p-2 text-red-400 hover:bg-red-900/20 rounded'>Logout</button>
        </div>
      )}
    </div>
  );
